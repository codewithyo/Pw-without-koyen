...
from config import api_id, api_hash, bot_token, auth_users, owner_id, admin_ids 
from datetime import datetime
import time
from concurrent.futures import ThreadPoolExecutor
from htmlx import handle_text_to_html, handle_platform_selection, process_text_content
THREADPOOL = ThreadPoolExecutor(max_workers=1000)
import logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

def clean_numeric_files():
    """Clean text files that start with numbers in the project directory"""
    try:
        ...
        for file in files:
            ...
            if re.match(r'^\d', file):
                try:
                    os.remove(file)
                    logging.info(f"Cleaned file: {file}")
                except OSError as e:
                    logging.error(f"Error deleting {file}: {e}")
    except Exception as e:
        logging.error(f"Error in clean_numeric_files: {e}")
...
#configs
CHANNEL_USERNAME = "HrXExtractor"  # Without @
CHANNEL_LINK = f"https://t.me/HrXExtractor"
CHANNEL_ID = -1002657483299
BACKUP_CHANNEL = 100265748329 # Add your backup channel ID here
...
@app.route('/')
def home():
    return "Bot is running! <b> https://t.me/HrXExtractor  Made by @Dreamm_ca </b>"
...
async def is_user_member(user_id: int) -> bool:
    try:
        ...
        return user_status.status not in [ChatMemberStatus.LEFT, ChatMemberStatus.BANNED]
    except Exception as e:
...
async def start(bot, message):
    ...
    try:
        ...
        logging.error(f"Error adding user to users.txt: {e}")

    random_image_url = random.choice(image_list)

    if not await is_user_member(user_id):
        keyboard = [
            [InlineKeyboardButton("🌟 Join Our Premium Channel 🌟", url="https://t.me/HrXExtractor")],
            [InlineKeyboardButton("✨ Verify Membership ✨", callback_data="check_join")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)

        await message.reply_animation(
            animation="https://media.giphy.com/media/3o7aCTPPm4OHfRLSH6/giphy.gif",
            caption="<b>🔒 Access Required</b>\n\n<i>Please join our exclusive channel to unlock premium features!</i>",
    ...
    keyboard = [
        [
            InlineKeyboardButton("⚡️ PW ⚡️", callback_data="pwwp"),
            InlineKeyboardButton("🎯 Cʟᴀssᴘʟᴜs 🎯", callback_data="cpwp")
        ],
    ...
    welcome_text = """
<b>🌟 Welcome to HR Extractor Bot 🌟</b>
<b>==========================================</b>
<i>Your gateway to seamless content extraction!</i>
...
<b> __WITHOUT PURCHASE APPX, PW, CP Extractor FREE__ </b>

<b> You can convert text to html for free </b>
"""

    await message.reply_animation(
        animation="https://media.giphy.com/media/l3q2IYN87QjIg51kc/giphy.gif",
...
async def check_join(bot, callback_query):
    if await is_user_member(callback_query.from_user.id):
        await callback_query.answer("✨ Welcome to Premium Access! ✨", show_alert=True)
        await callback_query.message.delete()

        keyboard = [
            [
                InlineKeyboardButton("⚡️ Pʜʏsɪᴄs Wᴀʟʟᴀʜ ⚡️", callback_data="pwwp"),
                InlineKeyboardButton("🎯 Cʟᴀssᴘʟᴜs 🎯", callback_data="cpwp")
            ],
            [InlineKeyboardButton("🔮 Aᴘᴘx 🔮", callback_data="appxwp")],
            [InlineKeyboardButton("📝 Tᴇxᴛ Tᴏ Hᴛᴍʟ(cp+pw) 📝", callback_data="text_to_html")],
        ...
        welcome_text = """
<b>🌟 Welcome to Premium Extractor Bot 🌟</b>
...
async def premium_info(bot, callback_query):
    premium_text = """
<b>✨ Premium Features Unlocked! ✨</b>

<i>Experience the power of our premium extractor:</i>
...
<b>💫 Start extracting now!</b>
"""
    await callback_query.answer("✨ Premium Features", show_alert=True)
...
@bot.on_callback_query(filters.regex("^next_page$"))
async def next_page(bot, callback_query):
    random_image_url = random.choice(image_list)
    keyboard = [
        [InlineKeyboardButton("🚀 CP ᴡɪᴛʜᴏᴜᴛ ᴘᴜʀᴄʜᴀsᴇ 🚀", callback_data="cpwp")],
        [InlineKeyboardButton("🚀 Aᴘᴘx ᴡɪᴛʜᴏᴜᴛ ᴘᴜʀᴄʜᴀsᴇ 🚀", callback_data="appxwp")],
        [InlineKeyboardButton("ᴄᴏᴍɪɴɪɴɢ... 🚀", callback_data="soon")],
        [InlineKeyboardButton("⬅️ ᴘʀᴇᴍɪᴜᴍ ᴍᴇɴᴜ ⬅️", callback_data="back_page")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
# Callback for back page
@bot.on_callback_query(filters.regex("^back_page$"))
async def back_page(bot, callback_query):
    random_image_url = random.choice(image_list)
    keyboard = [
        [InlineKeyboardButton("🇮🇳 ʙᴏᴛ ᴍᴀᴅᴇ ʙʏ 🇮🇳", url="https://t.me/Dreamm_ca")],
        [InlineKeyboardButton("🔔 ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ 🔔", url="https://t.me/HrXExtractor")],
        [InlineKeyboardButton("🚀 Pᴡ Wɪᴛʜᴏᴜᴛ Pᴜʀᴄʜᴀsᴇ 🚀", callback_data="pwwp")],
        [InlineKeyboardButton("➡️ ɴᴇxᴛ ➡️", callback_data="next_page")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)

    await callback_query.message.edit_caption(
        caption="👋 ʜᴇʏ\n\n☆ ɪ ᴀᴍ ᴀ ᴛxᴛ ᴇxᴛʀᴀᴄᴛᴏʀ ʙᴏᴛ.",
        reply_markup=reply_markup
    )
...
async def fetch_pwwp_data(session: aiohttp.ClientSession, url: str, headers: Dict = None, params: Dict = None, data: Dict = None, method: str = 'GET') -> Any:
    ...
    for attempt in range(max_retries):
        try:
            ...
            async with session.request(method, url, headers=request_headers, params=params, json=data) as response:
        ...
        if attempt < max_retries - 1:
            await asyncio.sleep(2 ** attempt)  # Changed from 90 to 2 for more reasonable retry delays
        else:
...
async def process_pwwp(bot: Client, m: Message, user_id: int):
    ...
    async with aiohttp.ClientSession(connector=CONNECTOR, loop=loop) as session:
        try:
            ...
            await editable.edit("**Enter Batch Name\n\nOR\n\nSend /id batch_id for extract batch**")
            try:
            ...
            # Check if input starts with /id for direct batch ID
            if batch_search.startswith('/id'):
                try:
                    batch_id = batch_search.split('/id')[1].strip()
                    if not batch_id:
                        raise Exception("Please provide batch ID after /id")
                    ...
                    if batch_details and batch_details.get("success"):
                        ...
                        selected_batch_name = batch_details["data"]["name"]
                        clean_batch_name = selected_batch_name.replace("/", "-").replace("|", "-")
                        clean_file_name = f"{user_id}_{clean_batch_name}"
                    else:
                        raise Exception("Invalid batch ID or no access to this batch")
                except Exception as e:
                    raise Exception(f"Error with batch ID: {str(e)}")
            else:
                # Search for batch by name
                page = 1
                all_courses = []
                while True:
                    url = f"https://api.penpencil.co/v3/batches/search?name={batch_search}&page={page}&limit=20"
                    courses = await fetch_pwwp_data(session, url, headers)
                    if not courses or not courses.get("data"):
                        break
                    
                    all_courses.extend(courses.get("data", []))
                    page += 1

                if not all_courses:
                    raise Exception("No batch found with this name")
                    
                # Show search results
                text = ''
                for cnt, course in enumerate(all_courses[:20], start=1):
                    name = course['name']
                    course_id = course['_id']
                    text += f"{cnt}. ```\nName : {name}\nID : {course_id}```\n"
                
                await editable.edit(f"**Found following batches matching your search.\nSend index number to download:\n\n{text}**")
                ...
                if raw_text4.isdigit() and 1 <= int(raw_text4) <= len(all_courses[:20]):
                    selected_course_index = int(raw_text4) - 1
                    course = all_courses[selected_course_index]
            ...
            # Show options menu for the selected batch
            ...
            if option == '1':
                ...
                raise Exception("Khazana option is work in progress")
            else:
                raise Exception("Invalid option selected")

            # Calculate time taken and prepare files
            end_time = time.time()
            ...
            # Prepare caption and send files
                    ...
                    with open(file, 'rb') as f:
                        ...
                        if file_ext == "txt":
                            try:
                                ...
                                logging.error(f"Failed to send backup to channel: {e}")
        ...
        try:
            ...
            async with session.get(f"https://{org_code}.courses.store", headers=hash_headers) as response:
                ...
                if hash_match:
                    ...
                    async with session.get(f"https://api.classplusapp.com/v2/course/preview/similar/{token}?limit=80", headers=headers) as response:
         ...
         #       'authorization': token,
            ...
            if courses:
                if total > 50:
                    ...
                    # Fix: Use UTF-8 encoding when writing files
...
async def platform_selection_callback(bot, callback_query):
    try:
        # Get platform from callback data
        platform = callback_query.data.split('_')[1]  # Will be 'pw' or 'cp'
        msg, _ = await handle_platform_selection(bot, callback_query)
        
        if msg:
            try:
                # Listen for the text file
                response = await bot.listen(chat_id=callback_query.message.chat.id, filters=filters.user(callback_query.from_user.id) & filters.document, timeout=300)
                if response.document:
                    # Process the text content with correct platform
                    await process_text_content(bot, response, platform)
                else:
                    await msg.reply_text("**Please send a text file.**")
            except ListenerTimeout:
                await msg.reply_text("**Timeout! You took too long to respond**")
            except Exception as e:
                await msg.reply_text(f"**Error: {str(e)}**")
    except Exception as e:
        await callback_query.message.reply_text(f"**Error: {str(e)}**")

@bot.on_message(filters.command(["add_auth"]))
async def add_auth(bot, message):
    user_id = message.from_user.id
    ...
    try:
        ...
        for i, line in enumerate(lines):
            if line.startswith('auth_users'):
                lines[i] = f'auth_users = {auth_users}\n'
                break
                
        with open('config.py', 'w') as f:
            f.writelines(lines)
            
        await message.reply_text(f"**Successfully added user {new_user_id} to authorized users.**")
    except Exception as e:
        await message.reply_text(f"**Error updating authorized users: {str(e)}**")

@bot.on_message(filters.command(["rem_auth"]))
async def rem_auth(bot, message):
    user_id = message.from_user.id
    ...
    # Get the user ID to remove from the command
    try:
        user_to_remove = int(message.text.split()[1])
    except (IndexError, ValueError):
        await message.reply_text("**Please provide a valid Telegram ID.\nExample: /rem_auth 2036109591**")
        return
        
    # Check if the user is in auth_users
    if user_to_remove not in auth_users:
        await message.reply_text(f"**User {user_to_remove} is not in the authorized users list.**")
        return
        
    # Remove the user from auth_users
    auth_users.remove(user_to_remove)
...
async def add_admin(bot, message):
    ...
    if user_id != owner_id:
        await message.reply_text("**You are not authorized to use this command. Only owner can use this command.**")
        return
        
    # Get the new admin ID from the command
    try:
        new_admin_id = int(message.text.split()[1])
    except (IndexError, ValueError):
        await message.reply_text("**Please provide a valid Telegram ID.\nExample: /add_admin 2036109591**")
        return
    ...
    # Add new admin to admin_ids
    admin_ids.append(new_admin_id)
    
    # Update config.py with new admin_ids
    try:
        with open('config.py', 'r') as f:
            lines = f.readlines()
        ...
        for i, line in enumerate(lines):
            if line.startswith('admin_ids'):
                lines[i] = f'admin_ids = {admin_ids}  # List of admin IDs\n'
                break
                
        with open('config.py', 'w') as f:
            f.writelines(lines)
            
        await message.reply_text(f"**Successfully added user {new_admin_id} as admin.**")
    except Exception as e:
        await message.reply_text(f"**Error updating admin: {str(e)}**")

@bot.on_message(filters.command(["rem_admin"]))
async def rem_admin(bot, message):
    ...
    # Get the admin ID to remove from the command
    try:
        admin_to_remove = int(message.text.split()[1])
    except (IndexError, ValueError):
        await message.reply_text("**Please provide a valid Telegram ID.\nExample: /rem_admin 2036109591**")
        return
        
    # Check if the user is in admin_ids
    if admin_to_remove not in admin_ids:
        await message.reply_text(f"**User {admin_to_remove} is not an admin.**")
        return
        
    # Remove admin from admin_ids
    ...
    try:
        ...
        await message.reply_text(f"**Successfully removed user {admin_to_remove} from admins.**")
    except Exception as e:
